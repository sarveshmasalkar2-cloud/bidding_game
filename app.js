...
const state = {
  ...
  canvasZoom:   1,
  currentColor: '#000000',
  currentTool:  'pencil',   // pencil | brush | broad | bucket | eraser
  brushSize:    6,
  isDrawing:    false,
  lastX:        0,
  lastY:        0,
...
async function handleRoomUpdate(room) {
  ...
  if (room.game_state === 'auction') {
    // Parse drawing order if it changed
    ...
    if (room.auction_phase === 'reveal') {
      ...
      startBiddingPhase();
    } else if (room.auction_phase === 'winner') {
      await showWinnerOverlay(room.current_auction_index);
    }
    return;
  }
...
// ── STEP 1: HOST STARTS GAME → push 'prompt' state ───────────────
...
// ── HOST: resolve bids for current lot ────────────────────────────
async function hostResolveBids(drawingId) {
  try {
    // Fetch all bids for this drawing
    ...
    if (bids && bids.length > 0) {
      ...
      // Update winner's balance (deduct bid)
      await db
        .from('players')
        .update({
          balance: db.rpc ? undefined : 0, // We'll fetch first, then update
        })
        .eq('room_id', state.roomId)
      ...
      if (winnerPlayer) {
        ...
        await db
          ...
          .update({
            balance:         newBalance,
            won_painting_ids: newPaintingIds,
          })
    ...
    // Push winner phase to room
    await db
      .from('rooms')
      .update({ auction_phase: 'winner' })
      .eq('id', state.roomId);
...
// ================================================================
function transitionToPrompt(prompt) {
  if (!prompt) return;

  // Only show prompt page if coming from lobby/drawing phases
  const currentPage = document.querySelector('.page.active')?.id;
  if (currentPage === 'page-5') return; // already drawing
  ...
  state.promptCountdown = setInterval(() => {
    ...
    if (count <= 0) {
      clearInterval(state.promptCountdown);
      // Host pushes drawing state; others wait for realtime update
      if (state.isHost) {
        hostStartDrawing();
      }
    }
...
function transitionToStudio(prompt) {
  showPage('page-5');
  document.getElementById('studio-prompt-display').textContent = prompt || 'Draw something!';
...
document.querySelectorAll('.brush-tool').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    canvas.style.cursor = (state.currentTool === 'bucket') ? 'cell' : 'crosshair';
  });
});
...
function updateBrushPreview() {
  ...
  preview.style.background = (state.currentTool === 'eraser') ? '#ffffff' : state.currentColor;
  preview.style.border = (state.currentTool === 'eraser') ? '1px solid #aaa' : 'none';
}
...
function applyBrushSettings() {
  ...
  switch (state.currentTool) {
    case 'pencil':
      size  = Math.max(1, state.brushSize * 0.5);
      break;
    case 'brush':
      blur  = 1;
      break;
    case 'broad':
      size  = state.brushSize * 2.5;
      cap   = 'square';
      break;
    case 'eraser':
      color = '#ffffff';
...
async function transitionToAuctionReveal(auctionIndex) {
  ...
  // Reveal: open curtains after a short pause
  setTimeout(() => {
    curtainL.classList.add('open');
    curtainR.classList.add('open');

    // After curtains finish, host pushes 'bidding' phase
    if (state.isHost) {
      setTimeout(async () => {
        await db.from('rooms')
...
// ── Winner Overlay ────────────────────────────────────────────────
async function showWinnerOverlay(auctionIndex) {
  clearInterval(state.bidTimerInterval);

  const drawing = state.currentAuctionDrawing;
  if (!drawing) return;

  // Fetch updated drawing with winner info
  const { data: updatedDrawing } = await db
    .from('drawings')
    .select('*')
    .eq('id', drawing.id)
    .single();

  const d = updatedDrawing || drawing;

  // Update our local balance if we won
  if (d.winner_nickname === state.nickname) {
  ...
  document.getElementById('artist-name').textContent =
    d.player_nickname ? 'By ' + d.player_nickname : 'Anonymous';

  // Populate winner overlay
  const winnerName = d.winner_nickname || 'No winner';
  const winBid     = d.winning_bid  || 0;

  document.getElementById('winner-name-bubble').textContent = winnerName;
  document.getElementById('winner-bid-display').textContent  = formatMoney(winBid);
  document.getElementById('winner-value-display').textContent = formatMoney(d.secret_value || '???');
  document.getElementById('winner-artist-display').textContent = d.player_nickname || 'Mystery';

  // Show overlay
  document.getElementById('winner-overlay').classList.remove('hidden');

  if (!d.winner_nickname) {
    document.getElementById('winner-name-bubble').textContent = 'No bids — going unsold!';
    document.getElementById('winner-bid-display').textContent = '$0';
  }

  // After 6 seconds, host advances to next lot
  if (state.isHost) {
    setTimeout(async () => {
      bidsThisRound = 0;
      document.getElementById('winner-overlay').classList.add('hidden');
...
async function transitionToResults() {
  ...
  const { data: drawings, error: dErr } = await db
    .from('drawings')
    .select('*')
    .eq('room_id', state.roomId);

  if (pErr || dErr) {
    console.error(pErr || dErr);
    return;
  }

  // Build a map of drawing id → secret_value
  const drawingMap = {};
  drawings?.forEach(d => { drawingMap[d.id] = d; });

  // Calculate final scores
  const scores = players.map(p => {
    const wonPaintingIds = p.won_painting_ids || [];
    const paintingValue  = wonPaintingIds.reduce((sum, id) => {
      return sum + (drawingMap[id]?.secret_value || 0);
    }, 0);
    const total = p.balance + paintingValue;
    return { ...p, paintingValue, total };
  });
  ...
  scores.forEach((s, i) => {
    ...
    li.innerHTML = `
      <span class="lb-rank ${i === 0 ? 'gold-rank' : ''}">${rankEmoji}</span>
      <div style="flex:1">
        <div class="lb-name">${s.nickname}</div>
        <div class="lb-breakdown">
          $${s.balance.toLocaleString()} remaining
          + $${s.paintingValue.toLocaleString()} in paintings
          ${s.nickname === state.nickname ? ' <span style="color:var(--gold)">← you</span>' : ''}
        </div>
      </div>
...
// ================================================================
(function init() {
  // Make sure default page is visible
  showPage('page-1');
  // Init brush preview with default color
  updateBrushPreview();
